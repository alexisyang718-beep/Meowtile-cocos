...
const HOME_BGM_PATH = 'audio/bgm/BGM_loading_to_home';
const HOME_BGM_VOLUME = 0.56;
const BGM_START_OFFSET_SECONDS = 1;
const LOADING_MEOW_PATH = 'audio/sfx/kitty_meow_loading';
const LOADING_MEOW_VOLUME = 0.56;
...
export class AppFlowController extends Component {
    ...
    private homeBgmSource: AudioSource | null = null;
    private appSfxSource: AudioSource | null = null;
    private homeBgmRequestId = 0;
    private homeBgmPath = '';
    ...
    protected start(): void {
        ...
        if (AppFlowController.DEBUG_SHOW_FPS_OVERLAY) this.buildFpsOverlay();
        void this.boot().catch((error) => this.handleBootError(error));
    }
    ...
    private getHomeBgmSource(): AudioSource {
        if (this.homeBgmSource?.node?.isValid) return this.homeBgmSource;
        const host = this.getUiHost();
        const node = host.getChildByName('HomeBGMSource') ?? new Node('HomeBGMSource');
        if (!node.parent) {
            node.layer = host.layer;
            host.addChild(node);
        }
        this.homeBgmSource = node.getComponent(AudioSource) ?? node.addComponent(AudioSource);
        return this.homeBgmSource;
    }

    private getAppSfxSource(): AudioSource {
        if (this.appSfxSource?.node?.isValid) return this.appSfxSource;
        const host = this.getUiHost();
        const node = host.getChildByName('AppSfxSource') ?? new Node('AppSfxSource');
        if (!node.parent) {
            node.layer = host.layer;
            host.addChild(node);
        }
        this.appSfxSource = node.getComponent(AudioSource) ?? node.addComponent(AudioSource);
        return this.appSfxSource;
    }

    private playHomeBgm(restart = false): void {
        const source = this.getHomeBgmSource();
        const requestId = ++this.homeBgmRequestId;
        if (restart) {
            source.stop();
            this.homeBgmPath = '';
        }
        if (!restart && this.homeBgmPath === HOME_BGM_PATH && source.clip) {
            source.volume = HOME_BGM_VOLUME;
            if (!source.playing) source.play();
            return;
        }
        resources.load(HOME_BGM_PATH, AudioClip, (error, clip) => {
            if (error || !clip || requestId !== this.homeBgmRequestId) return;
            this.homeBgmPath = HOME_BGM_PATH;
            source.clip = clip;
            source.loop = true;
            source.volume = HOME_BGM_VOLUME;
            source.currentTime = Math.min(BGM_START_OFFSET_SECONDS, Math.max(0, clip.getDuration() - 0.1));
            source.play();
        });
    }

    private stopHomeBgm(): void {
        this.homeBgmRequestId++;
        this.homeBgmSource?.stop();
        this.homeBgmPath = '';
    }

    private playLoadingMeow(): void {
        const source = this.getAppSfxSource();
        resources.load(LOADING_MEOW_PATH, AudioClip, (error, clip) => {
            if (error || !clip || !source.node.isValid) return;
            source.playOneShot(clip, LOADING_MEOW_VOLUME);
        });
    }
    ...
    private handleBootError(error: unknown): void {
        console.error('[AppFlowController] boot failed', error);
        this.loadingRoot.active = false;
        ...
        try {
            this.homeView.show(1);
        } catch (fallbackError) {
            console.error('[AppFlowController] home fallback failed', fallbackError);
        }
    }

    private async showLoading(message?: string): Promise<void> {
        this.currentScene = 'loading';
        this.playHomeBgm(true);
    ...
    private animateLoadingProgress(fill: Node, width: number, height: number): Promise<void> {
        ...
        return new Promise<void>((resolve) => {
            const updateProgress = (): void => {
                ...
                if (!meowPlayed && ratio >= 0.99) {
                    meowPlayed = true;
                    this.playLoadingMeow();
                }
